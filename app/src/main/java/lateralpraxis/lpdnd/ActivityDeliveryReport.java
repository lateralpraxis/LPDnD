			...
			private class AsyncDeliveryListWSCall extends AsyncTask<String, Void, String> {
				...
				protected String doInBackground(String... params) {
					try {
						...
						String[] value = {"ReadDelivery",lang,((CustomType)spCentre.getSelectedItem()).getId(),userId,  dateString, ((CustomType)spCompany.getSelectedItem()).getId()};
						responseJSON = "";
	...
	private class AsyncCompanyWSCall extends AsyncTask<String, Void, String> {
		...
		protected void onPostExecute(String result) {
			try {
				...
				common.showAlert(ActivityDeliveryReport.this, "Company Downloading failed: " + e.toString(), false);
			}
			Dialog.dismiss();
		}
