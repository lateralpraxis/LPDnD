...
public class ActivityDemandReport  extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//Code to set layout
		setContentView(R.layout.activity_demand_report);
		...
		listDemand = (ListView) findViewById(R.id.listDemand);
...
//				}
	...
	private class AsyncCentreWSCall extends AsyncTask<String, Void, String> {
		private ProgressDialog Dialog = new ProgressDialog(ActivityDemandReport.this);
		...
		protected void onPostExecute(String result) {
			try {
				if (!result.contains("ERROR: ")) {
					...
					common.showAlert(ActivityDemandReport.this, result, false);
				}
			} catch (Exception e) {
				common.showAlert(ActivityDemandReport.this, "Centre Downloading failed: " + e.toString(), false);
			}
			Dialog.dismiss();
		}
	...
	public static class ViewHolder {
		TextView tvName, tvProduct, tvQuantity;
	...
	public class CustomAdapter extends BaseAdapter {
		...
		public View getView(final int arg0, View arg1, ViewGroup arg2) {
			...
			if (arg1 == null) {
				arg1 = mInflater.inflate(R.layout.list_demand_details, null);
				...
				holder.llCustomer = (LinearLayout) arg1.findViewById(R.id.llCustomer);
			...
			if(list.get(arg0).get("Flag").equalsIgnoreCase("1"))
				holder.llCustomer.setVisibility(View.GONE);
			else
				holder.llCustomer.setVisibility(View.VISIBLE);
	...
	//<editor-fold desc="Async class Class to handle fetch Demand web service call as separate thread">
	private class AsyncDemandListWSCall extends AsyncTask<String, Void, String> {
		private ProgressDialog Dialog = new ProgressDialog(ActivityDemandReport.this);
		...
		protected void onPostExecute(String result) {
			try {
				if (!result.contains("ERROR: ")) {
					...
					if (listSize != 0) {
						listDemand.setAdapter(new CustomAdapter(mContext, wordList));

						ViewGroup.LayoutParams params = listDemand.getLayoutParams();
				...
				common.showAlert(ActivityDemandReport.this, "Demand Downloading failed: " + "Unable to get response from server.", false);
			}
			Dialog.dismiss();
		}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.action_go_to_home: 
			...
			if(userRole.contains("System User") || userRole.contains("Centre User") || userRole.contains("MIS User") || userRole.contains("Management User") && (!userRole.contains("Route Officer") || !userRole.contains("Collection Officer") || !userRole.contains("Accountant")))
				i = new Intent(ActivityDemandReport.this,ActivityAdminHomeScreen.class);
			else
				i = new Intent(ActivityDemandReport.this,ActivityHomeScreen.class);
			startActivity(i);
	...
	public void onBackPressed() {
		Intent i = new Intent(ActivityDemandReport.this,ActivityReport.class);
		startActivity(i);
		i.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
		startActivity(i);
		finish();
	...
	private class AsyncCompanyWSCall extends AsyncTask<String, Void, String> {
		...
		protected void onPostExecute(String result) {
			try {
				...
				common.showAlert(ActivityDemandReport.this, "Company Downloading failed: " + e.toString(), false);
			}
			Dialog.dismiss();
		}
