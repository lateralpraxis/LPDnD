...
public class ActivityCashCollectReport   extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//Code to set layout
		setContentView(R.layout.activity_cash_collect_report);
...
//				}
	...
	private class AsyncCompanyWSCall extends AsyncTask<String, Void, String> {
		private ProgressDialog Dialog = new ProgressDialog(ActivityCashCollectReport.this);
		...
		protected void onPostExecute(String result) {
			try {
				if (!result.contains("ERROR: ")) {
					...
					common.showAlert(ActivityCashCollectReport.this, result, false);
				}
			} catch (Exception e) {
				common.showAlert(ActivityCashCollectReport.this, "Company Downloading failed: " + e.toString(), false);
			}
			Dialog.dismiss();
		}
	...
	public class CustomAdapter extends BaseAdapter {
		...
		public View getView(final int arg0, View arg1, ViewGroup arg2) {
			...
			if (arg1 == null) {
				arg1 = mInflater.inflate(R.layout.list_collect_report, null);
				holder = new ViewHolder();

				holder.tvCollBy = (TextView) arg1.findViewById(R.id.tvCollBy);
			...
			if(list.get(arg0).get("Type").equalsIgnoreCase("Sale At Plant") || list.get(arg0).get("Type").equalsIgnoreCase("Customer Payment") || list.get(arg0).get("Type").equalsIgnoreCase("Route Officer"))
				holder.tvCollBy.setText("  "+list.get(arg0).get("Type"));
			else
				holder.tvCollBy.setText(Html.fromHtml("<b>"+list.get(arg0).get("Type")+"</b>"));
			if(list.get(arg0).get("Amount").equalsIgnoreCase("0.0"))
	...
	//<editor-fold desc="Async class Class to handle fetch Cash Deposit web service call as separate thread">
	private class AsyncCashDepositListWSCall extends AsyncTask<String, Void, String> {
		private ProgressDialog Dialog = new ProgressDialog(ActivityCashCollectReport.this);

		@Override
		protected String doInBackground(String... params) {
			try {
				String[] name = {"action", "lang", "compId", "userId", "date"};
				String dateString;
				Date date = new Date();
				date = dateFormatter_display.parse(tvDate.getText().toString().trim());
				dateString = dateFormatter_database.format(date);
				String[] value = {"ReadCashCollection",lang,((CustomType)spCompany.getSelectedItem()).getId(),userId,  dateString};
				responseJSON = "";
				//Call method of web service to download data from server
				responseJSON = common.CallJsonWS(name, value, "ReadCompanyAndDateWiseReports", common.url);
				return "";
			} catch (SocketTimeoutException e) {
				return "ERROR: TimeOut Exception. Either Server is busy or Internet is slow";
			} catch (final Exception e) {
		...
		protected void onPostExecute(String result) {
			try {
				if (!result.contains("ERROR: ")) {
					...
					if (listSize != 0) {
						listCollection.setAdapter(new CustomAdapter(mContext, wordList));

						ViewGroup.LayoutParams params = listCollection.getLayoutParams();
				...
				common.showAlert(ActivityCashCollectReport.this, "Cash Deposit Downloading failed: " + "Unable to get response from server.", false);
			}
			Dialog.dismiss();
		}
		...
		@Override
		protected void onPreExecute() {
			Dialog.setMessage("Downloading Cash Deposit..");
			Dialog.setCancelable(false);
			Dialog.show();
		}
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.action_go_to_home: 
			...
			if(userRole.contains("System User") || userRole.contains("Centre User") || userRole.contains("MIS User") || userRole.contains("Management User") && (!userRole.contains("Route Officer") || !userRole.contains("Collection Officer") || !userRole.contains("Accountant")))
				i = new Intent(ActivityCashCollectReport.this,ActivityAdminHomeScreen.class);
			else
				i = new Intent(ActivityCashCollectReport.this,ActivityHomeScreen.class);
			startActivity(i);
	...
	public void onBackPressed() {
		Intent i = new Intent(ActivityCashCollectReport.this,ActivityReport.class);
		startActivity(i);
		i.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
		startActivity(i);
		finish();
