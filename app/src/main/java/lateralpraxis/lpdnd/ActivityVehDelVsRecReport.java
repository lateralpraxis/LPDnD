...
public class ActivityVehDelVsRecReport extends Activity{
	...
	//On create method similar to page load
		...
		protected void onCreate(Bundle savedInstanceState) {
			...
			//Code to set layout
			setContentView(R.layout.activity_veh_delvsrec_report);
			...
			listDelRec = (ListView) findViewById(R.id.listDelRec);
...
//					{
						if (common.isConnected()) {
							AsyncVehiclewiseDeliveryReceiptListWSCall task = new AsyncVehiclewiseDeliveryReceiptListWSCall();
							task.execute();
						}
//					}
				}
		...
		private class AsyncCentreWSCall extends AsyncTask<String, Void, String> {
			private ProgressDialog Dialog = new ProgressDialog(ActivityVehDelVsRecReport.this);
			...
			protected void onPostExecute(String result) {
				try {
					if (!result.contains("ERROR: ")) {
						...
						common.showAlert(ActivityVehDelVsRecReport.this, result, false);
					}
				} catch (Exception e) {
					common.showAlert(ActivityVehDelVsRecReport.this, "Centre Downloading failed: " + e.toString(), false);
				}
				Dialog.dismiss();
			}
		...
		public static class ViewHolder {
			TextView tvName, tvDelRec,tvAllocId,tvVehicle,tvRouteOfficer;
		...
		public class CustomAdapter extends BaseAdapter {
			...
			public View getView(final int arg0, View arg1, ViewGroup arg2) {
				...
				if (arg1 == null) {
					arg1 = mInflater.inflate(R.layout.list_veh_delvsrec_report, null);
					holder = new ViewHolder();

					holder.tvName = (TextView) arg1.findViewById(R.id.tvName);
					holder.tvDelRec = (TextView) arg1.findViewById(R.id.tvDelRec);
					holder.tvAllocId = (TextView) arg1.findViewById(R.id.tvAllocId);
				...
				holder.tvAllocId.setText("AL"+list.get(arg0).get("AllocId"));
				holder.tvVehicle.setText(list.get(arg0).get("Vehicle"));
				holder.tvRouteOfficer.setText(list.get(arg0).get("RouteOfficer"));
				holder.tvName.setText(list.get(arg0).get("Customer"));
				String rcptAmt=list.get(arg0).get("Receipt").equalsIgnoreCase("0.0")?"-":common.convertToTwoDecimal(list.get(arg0).get("Receipt"));
				holder.tvDelRec.setText(common.convertToTwoDecimal(list.get(arg0).get("Amount"))+"/"+rcptAmt);
		...
		//<editor-fold desc="Async class Class to handle fetch Vehiclewise Delivery Receipt web service call as separate thread">
		private class AsyncVehiclewiseDeliveryReceiptListWSCall extends AsyncTask<String, Void, String> {
			private ProgressDialog Dialog = new ProgressDialog(ActivityVehDelVsRecReport.this);
			...
			protected String doInBackground(String... params) {
				try {
					...
					String[] value = {"ReadVehiclewiseDeliveryReceipt",lang,((CustomType)spCentre.getSelectedItem()).getId(),userId,  dateString, ((CustomType)spCompany.getSelectedItem()).getId()};
					responseJSON = "";
			...
			protected void onPostExecute(String result) {
				try {
					if (!result.contains("ERROR: ")) {
						...
						if (jsonArray.length() > 0) {
							for (int i = 0; i < jsonArray.length(); ++i) {
								...
								map.put("AllocId", jsonArray.getJSONObject(i)
										.getString("A"));
								map.put("DelId", jsonArray.getJSONObject(i)
										.getString("B"));
								map.put("Vehicle", jsonArray.getJSONObject(i)
										.getString("C"));
								map.put("RouteOfficer", jsonArray.getJSONObject(i)
										.getString("D"));
								map.put("Customer", jsonArray.getJSONObject(i)
										.getString("E"));
								map.put("Amount", jsonArray.getJSONObject(i)
										.getString("F"));
								map.put("Receipt", jsonArray.getJSONObject(i)
										.getString("G"));
						...
						if (listSize != 0) {
							listDelRec.setAdapter(new CustomAdapter(mContext, wordList));

							ViewGroup.LayoutParams params = listDelRec.getLayoutParams();
					...
					common.showAlert(ActivityVehDelVsRecReport.this, "Vehicle wise Delivery VS Receipt Report Downloading failed: " + "Unable to get response from server.", false);
				}
				Dialog.dismiss();
			}
			...
			@Override
			protected void onPreExecute() {
				Dialog.setMessage("Downloading Vehicle wise Delivery VS Receipt Report..");
				Dialog.setCancelable(false);
				Dialog.show();
			}
		}
		...
		public boolean onOptionsItemSelected(MenuItem item) {
			...
			case R.id.action_go_to_home: 
				...
				if(userRole.contains("System User") || userRole.contains("Centre User") || userRole.contains("MIS User") || userRole.contains("Management User") && (!userRole.contains("Route Officer") || !userRole.contains("Collection Officer") || !userRole.contains("Accountant")))
					i = new Intent(ActivityVehDelVsRecReport.this,ActivityAdminHomeScreen.class);
				else
					i = new Intent(ActivityVehDelVsRecReport.this,ActivityHomeScreen.class);
				startActivity(i);
		...
		public void onBackPressed() {
			Intent i = new Intent(ActivityVehDelVsRecReport.this,ActivityReport.class);
			startActivity(i);
			i.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
			startActivity(i);
			finish();
	...
	private class AsyncCompanyWSCall extends AsyncTask<String, Void, String> {
		...
		protected void onPostExecute(String result) {
			try {
				...
				common.showAlert(ActivityVehDelVsRecReport.this, "Company Downloading failed: " + e.toString(), false);
			}
			Dialog.dismiss();
		}
